...
GameManager::GameManager(CommonData* commonData) 
{
...
bool GameManager::CheckGameOver()	//바닥 부분 점검해서 게임오버인지 확인
{
	for (int i = 0; i < (commonData->GetMapSizeX() - 1); i++)
	{
		switch (commonData->GetBoard(i, commonData->GetMapSizeY() - 2)) {
		case OCTOPUS:
		case CRAB:
		case SQUID:
			return true;
