...
Player::Player(CommonData* commonData)
{
...
void Player::Move() 
{
	if (_kbhit())						//키보드누르면
	{
		...
		else if (32 == key)		//스페이스바 : 총알발사
		{
			...
			bullet = new Bullet(position.GetX(), position.GetY(), true, commonData);		//총알생성
...
void Player::Shoot()
{
	...
	switch (isCrash)						//충돌한놈의 정보 기록
	{
		...
		default:
			hitInfo[0] = true;
			hitInfo[1] = bullet->GetXPos();
			hitInfo[2] = bullet->GetYPos();
			hitInfo[3] = isCrash;
			RemoveBullet();
	}
	isCrash = -1;
...
void Player::RemoveBullet()
{
	...
	if (-1 == commonData->GetBoard(bullet->GetXPos(), bullet->GetYPos()))		//총알위치값이 제대로 전송됬는지 검사
		return;

	if(commonData->GetBoard(bullet->GetXPos(), bullet->GetYPos()) != WALL)
		commonData->SetBoard(bullet->GetXPos(), bullet->GetYPos(), BLANK);
	isShoot = false;
	shootable = true;
	delete bullet;
	bullet = NULL;
